...
bloom_filter = BloomFilter(100000, 0.01)  # Adjust parameters as needed

# Initialize the hash map
hash_map = HashMap()

# Initialize the Trie spell checker
grp = {
...
# Function to populate the Bloom filter and hash map with initial keywords and URLs
def populate_index():
    keywords_urls = {
    "python": [
...
# Function to add a keyword or update existing keyword with additional URLs
def add_keyword():
    keyword = add_entry.get()
    urls = url_entry.get().split(",")  # Split URLs by comma if multiple URLs are provided
